...
function tidyUpString(arr) {
  return arr.map(x => x.split("/").join(" ").trim().toLowerCase());
...
function remove(arr, index) {
  ...
  const value = arr2.splice(index, 1);

  return arr2.filter(x => x !== value);
...
function formatPercentage(percent) {
  return percent.map(x => {
    if (x < 100) {
      return parseFloat(x.toFixed(2)).toString().concat("%")
